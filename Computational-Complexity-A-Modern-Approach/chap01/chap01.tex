...
% We define logical expression $P(x,i)$ such that $P(x,i)$ is true iff $x=p_i$. Then 
%			$$P(x,i)=(\forall_p(PRIME(p)\wedge((C+i)^2<p<(C+i+1)^2))\wedge (x\geq p))\wedge PRIME(x)$$
%			$$BIT(n,i)=(x\geq 1)\wedge P(x,i)\wedge DIVIDES(x,n)$$
$$
